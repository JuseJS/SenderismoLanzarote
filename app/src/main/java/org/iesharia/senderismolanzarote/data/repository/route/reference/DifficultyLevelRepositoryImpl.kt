...
import org.iesharia.senderismolanzarote.data.mapper.route.reference.toDifficultyLevelEntity
...
class DifficultyLevelRepositoryImpl @Inject constructor(
    ...
    override fun getAllDifficultyLevels(): Flow<List<DifficultyLevelModel>> {
        return difficultyLevelDao.getAllDifficultyLevels()
            .map { entities ->
    ...
    override suspend fun insertDifficultyLevel(difficultyLevel: DifficultyLevelModel) {
        difficultyLevelDao.insertDifficultyLevel(difficultyLevel.toDifficultyLevelEntity())
    }
}